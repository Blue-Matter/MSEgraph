...
summary_df <- function(x, quantiles=c(0.025, 0.975), func=get_SSB, scale=NULL, years=NULL) {
  ...
  if (!is.null(df[['MP']])) {
    ...
    if (!is.null(df[['Fleet']])) {
      if (!is.null(df[['Age']])) {
        df <- df %>%  group_by(Year,Age, Variable, Period, Model, Fleet)
      } else {
...
plot_Biomass <- function(x, ...) {
  plot_TS(x, get_function=get_Biomass, ...)
}

#' @export
#' @describeIn plot_TS Plot the Landings (biomass)
plot_Landings <- function(x, ...) {
  plot_TS(x, get_function=get_Landings, ...)
}
...
plot_Removals <- function(x, ...) {
  plot_TS(x, get_function=get_Removals, ...)
}

#' @export
#' @describeIn plot_TS Plot the Recruits (numbers)
plot_Recruits <- function(x, ...) {
  plot_TS(x, get_function=get_Recruits, ...)
}
